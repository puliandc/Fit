...
class WorkoutViewModel: ObservableObject
{
    ...
    private var _lastExerciseId: UUID = .init()
    ...
    {
        guard currentExerciseIndex < workoutPlan.exercises.count
        else
        {
    ...
    private func updateCurrentSetDisplay()
    {
        ...
        // 从缓存中获取当前练习的所有组
        guard let exerciseSetsForThisExercise = _exerciseGroupsCache[currentExerciseId]
        else
        {
            currentSet = 1
            return
    ...
    @objc private func updateAllTimers()
    {
        ...
        if isResting, timeLeft > 0
        {
            ...
            if timeLeft == 0
            {
                isResting = false
                // 完成检查：若已完成则不再启动下一个动作
                if isWorkoutComplete || workoutFinished
                {
                    completeWorkoutIfNeeded()
                }
                else
    ...
    {
        guard let nextExerciseInfo = getNextExerciseInfo()
        else
        {
            return
        }
    ...
    private func debouncedUIUpdate()
    {
        ...
        // 创建新的更新任务
        uiUpdateWorkItem = DispatchWorkItem
        { [weak self] in
            guard let self = self else { return }
