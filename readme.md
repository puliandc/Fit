...
//updated by Jason Lu on 12:20:00 10/12/2025 - 修正时间戳，完善时间引用规范说明
...
**时间戳格式**：
- **标准格式**：`//created by Jason Lu on HH:MM:SS MM/DD/YYYY`
- **示例**：`//updated by Jason Lu on 12:20:00 10/12/2025`
- **时间基准**：使用当前系统时间（24小时制）
- **日期格式**：月/日/年（MM/DD/YYYY）

**AI助手使用指南**：
1. **检查当前时间**：更新文档前请确认当前系统时间
2. **使用24小时制**：时间格式为 HH:MM:SS（小时:分钟:秒）
3. **日期顺序**：严格按照 月/日/年 的顺序
4. **更新描述**：在时间戳后添加简短的更新内容说明
5. **保持一致性**：所有时间戳必须遵循相同格式

**正确示例**：
```swift
//created by Jason Lu on 09:17:00 10/12/2025
//updated by Jason Lu on 14:30:15 10/12/2025 - 修复WorkoutScreen布局问题
//updated by Jason Lu on 16:45:20 10/12/2025 - 集成Figma设计资源
```

**错误示例及修正**：
```swift
// ❌ 错误：时间格式不正确
//updated by Jason Lu on 18:50:00 10/12/2025

// ✅ 正确：使用当前准确时间
//updated by Jason Lu on 12:20:00 10/12/2025 - 修正时间戳格式
```
